...
8.2 in~\cite{hackbusch}.  First, we examine the regularity of the kernel
of the double layer operator
$\underline{\mbox{curl}}_{\S}\,G(x,y)\cdot\vec{t}(y)$.  In particular,
we establish that it is continuous at $x \in \Gamma$.
...
Now that we have established that the kernel of the double layer
operator is continuous, we prove can establish the jump relations of the
...
$\Omega^{c}$) angle of $\Gamma$ at $x_0$. For a smooth curve, $\alpha =
