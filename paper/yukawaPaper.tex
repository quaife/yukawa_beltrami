...
\mathbb{R}$ is constant. This  problem arises in
the context of solving the isotropic heat equation via Rothe's method: first apply a semi-implicit time discretization to the heat equation and then time-stepping involves repeatedly solve an elliptic PDE  of the form~\eqref{bigmodel}. In this context, $k^2 = O(1/\Delta t)$, and with this in mind, we restrict our attention to the case $k>1/2$.
This approach is discussed in \cite{rothe:heat} for the heat equation in the plane. 
...
Before proceeding, we first fix notation.  A point on the sphere $\S$ is
...
\begin{align*}
  ...
  \right) \in{\S}, \quad \varphi \in [0,2\pi), 
    \quad \theta \in [0,\pi].
\end{align*}
Recall that in spherical coordinates, $\lap_\S$ is defined as
\begin{align*}
  \lap_S u(x) \, = \, \left[
...
For two points $x,y$  on the unit sphere, with
spherical coordinates $x=x(\phi,\theta), y=(\alpha,\beta)$ and Cartesian coordinates $(x_1,x_2,x_2),(y_1,y_2,y_3)$, then the solid angle between them is given by \begin{align*}
...
Suppose, again without loss of generality, that $x_0=(0,0,1)$.  Let $x$ be
characterized by the spherical angles $\phi, \theta$, that is,
...
\end{align}
The expression in~\eqref{curlT} will be important in subsequent
sections, and is precisely the kernel of the second layer operator. We
shall define this carefully later on. For now, however, we are
interested in the limit of $
\underline{\mbox{curl}}_{\S} G_k(x,x_0)\cdot \vec{t}$ as the point
$x\rightarrow x_0=(0,0,1)$. For this argument, assume that
$x=x(\theta,0)$ is the curve along which we approach $x_0$. 
...
\begin{lemma} 
\label{l:diagTerm} 
Let $x_0,x'$ be points on the sphere, lying on a smooth curve $\Gamma=
(x'(\theta),0)$. Let $x_0$ be fixed. Then
\begin{align*}
  \lim_{\substack {x' \rightarrow x_{0} \\ y \in \Gamma}}
    [\underline{\mbox{curl}}_{\S}G_{k}(x_{0},y) \cdot \vec{t}(x')] = 
      -\frac{1}{2\pi}.
  \end{align*}
  ...
  \begin{align*}
      \lim_{\substack {x' \rightarrow x_{0} \\ x' \in \Gamma}}
        ...
        -\frac{1}{4\pi} \kappa \mathbf{s}\cdot(\mathbf{n}_p\times \mathbf{x}_0)
    \end{align*}
Here $\kappa$ is the curvature \todo{which one?} of $\Gamma$ at $x_0$.
\\
\todo{MC: This is the Lemma I am hoping you can address and prove.  It
would be nice if there is no curvature in the result and everything is
in terms of $x$ and its first and second derivatives}
 \end{lemma}
\begin{proof}
