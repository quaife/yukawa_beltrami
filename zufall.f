...
* c
ccc      program fibotest
ccc      implicit none
ccc      integer n
ccc      parameter(n=5000)
c
...
c
ccc      double precision a(n)
ccc      double precision t1,t2,second
ccc      integer p(n)
ccc      integer seed
cccc
cccc  initialize the seeds for the uniform random number generator
cccc
ccc      seed = 0
ccc      t1 = second()
ccc      call zufalli(seed)
ccc      t2 = second()
ccc      t1 = t2 - t1
ccc      print 100
ccc      print 200
ccc      print 300,t1
cccc
cccc  first do the uniform distribution test
cccc
ccc      print 400
ccc      call zufallt(n,a)
cccc
cccc  next, the box-muller gaussian generator test
cccc
ccc      print 500
ccc      call normalt(n,a)
cccc
cccc  finally, the poisson distribution test yields integers
cccc
ccc      print 600
ccc      call fischet(n,p)
cccc
ccc      print 700
cccc
ccc100   format(/15x,24('=')/
ccc     *   15x,'===== BEGIN TESTS  ====='/,
ccc     *   15x,24('='))
ccc200   format(/1x,36('-')/
ccc     *   1x,'***** Initialization of zufall *****'/,
ccc     *   1x,36('-'))
ccc300   format(/1x,' Initialization sequence takes ',e12.3,' seconds')
ccc400   format(/1x,37('-')/
ccc     *   1x,'***** Uniform distribution test *****'/,
ccc     *   1x,37('-')/)
ccc500   format(/1x,38('-')/
ccc     *   1x,'***** Gaussian distribution test *****'/,
ccc     *   1x,37('-')/)
ccc600   format(/1x,38('-')/
ccc     *   1x,'***** Poisson distribution test *****'/,
ccc     *   1x,37('-')/)
ccc700   format(/15x,24('=')/
ccc     *   15x,'===== END OF TESTS ====='/,
ccc     *   15x,24('=')/)
ccc      stop
ccc      end
c
